...
import degreeOptions from "../data/degrees.json";
...
export const DegreeList = ({
    ...
    // Expands degree plan when one is clicked on in the list of degrees
    const handleDegreeView = (id: number) => {
        setDisplayId(id);
    };

    // Exits out of the currently viewed degree
    const resetDegreeView = () => {
        setDisplayId(null);
    };

    // Show/Hide the UI for adding new degrees
    const handleAddClick = () => {
        setIsAdding(!isAdding);
    };

    // Handles the adding of new degrees based on what the user entered.
    ...
    /*
        Function Generated by ChatGPT. Uses Blob to download the users Degree Plan List as a JSON file.
    */
    ...
    /*
        Function Generated by ChatGPT. Reads the JSON file uploaded.
    */
    ...
    // Updates the globalDegreeList with degrees from the uploaded JSON file.
    function handleUpload(degrees: Degree[]) {
        uploadDegrees(degrees);
    }

    // Clears the user's local storage of all data (Degrees) stored.
    const clearLocalStorage = () => {
        localStorage.clear();
    };

    // Updates the user's selected degree plan (CISC BS, CISC BA, etc)
    // when they select it in the drop down menu
